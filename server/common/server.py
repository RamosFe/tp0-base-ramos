...
class Server:
    def __init__(self, port, listen_backlog):
        ...
        self._store_manager = StoreManager(MAX_AGENCIES)
    ...
    def run(self):
        ...
        """
        multiprocessing.Manager().register('store', StoreManager)

        with multiprocessing.Pool() as pool:
            with multiprocessing.Manager() as manager:
                store = manager.store(MAX_AGENCIES)

                while self._running:
                    try:
                        client_sock = self.__accept_new_connection()
                        self._active_clients.append(client_sock)
                        pool.apply_async(handle_client_connection, (client_sock, store))
...
def handle_client_connection(client_sock, store):
    """
